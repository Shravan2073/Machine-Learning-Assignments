...
def gini(series: pd.Series, treat_continuous_with_bins: bool = True, n_bins: int = 4):
    s = series.dropna()
    if s.empty:
        return 0.0
    ...
    counts = Counter(s)
    return gini_from_counts(counts.values())

def main():
    parser = argparse.ArgumentParser(description="A2: Calculate Gini index of the target column (bin continuous targets if needed).")
    ...
    G = gini(df[target], treat_continuous_with_bins=True, n_bins=args.bins)
    print(f"Gini index of target '{target}' = {G:.6f}")
